...
export const saveFile = async (blob, parentFolder, keyString = "") => {
    try {
        if (!blob) {
...
export const deliverFileOld = async (filename) => {
    try {
        let { Body } = await s3Client.send(new GetObjectCommand({
            Bucket: bucketName,
            Key: filename,
        }));
        if (!Body) {
...
export const deliverFile = async (filename) => {
    try {
